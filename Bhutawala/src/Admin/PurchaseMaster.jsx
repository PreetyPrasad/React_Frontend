...
export default function PurchaseMaster() {
  ...
  const fetchPurchaseMasters = async () => {
    try {
      ...
      if (response.status.toUpperCase() === "OK") {
        ...
        // Map purchase master data with supplier and year names
  ...
  // const GrossTotalTemplate = (purchaseMaster) => {
  // return `${material.PurchaseDate} ${material.GrossTotal}`
  // };

  const editTemplate = (purchaseMaster) => {
    return <i onClick={() => fetchPurchaseMasterDetail(purchaseMaster.purchaseId)} className='fas fa-edit'></i>;
  };

  const deleteTemplate = (purchaseMaster) => {
    return <i onClick={() => deletePurchaseMaster(purchaseMaster.purchaseId)} className='fas fa-trash'></i>;
  };
  ...
  const formatDate = (dateString) => {
    ...
    if (isNaN(date)) return dateString; // If invalid date, return as is
        ...
        <div className="col-lg-12">
          <div className="card">
            <div className="card-header">
              <h4 className="card-title mb-0">PurchaseMaster</h4>
            </div>
            <div className="card-body">
              <div className="row">
                ...
                <div className="col-md-12 table-responsive">
                  <DataTable showGridlines size='small'
                    loading={dataLoading} value={PurchaseMasters}
                    tableStyle={{ minWidth: '50rem' }}>
                    <Column style={{ width: "100px" }}
                      field="purchaseId" header="#"></Column>
                    <Column field="supplierName" header="Supplier"></Column>
                    <Column field="billNo" header="billNo"></Column>
